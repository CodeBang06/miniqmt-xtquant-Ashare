# Ashare-tushare-qmt-xtquant

1、Ashare（开源、极简的A股实时行情数据API），完全免费的沪深股票行情数据，Python最简封装接口，包含日线,历史K线,分时线,分钟线,全部实时采集，系统包括新浪腾讯双数据核心采集获取，自动故障切换，STOCK数据格式成DataFrame格式,可用来查询研究量化分析，股票程序自动化交易系统.为量化研究者在数据获取方面极大地减轻工作量，更加专注于策略和模型的研究与实现。  
2、tushare国内主流、面向量化爱好者的机构化金融数据API接口平台，这里提供一套可以免费使用的token，详情看后面的介绍。
3、MiniQMT 轻量级券商专业级量化交易终端 + XtQuant Python API的Python量化交易框架，是目前最适合普通股民的本地实盘量化方案，低延迟、高稳定、全私密，包括环境搭建、行情获取、api使用、策略开发、实盘自动交易部署，专为A股交易者打造；  

<img width="171" height="220" alt="git交流群" src="https://github.com/user-attachments/assets/7e4ba6ae-ec0b-4986-8fa5-d330555293d7" />

> #### 扫一扫，即可获取任一家国金、国信、湘财、国盛、长城的qmt回测模拟账号。
> #### 扫一扫，即可获取低门槛5万、低佣金万0.75开通qmt自动化交易实盘权限。
> #### 股市程序化交易大群, 圈内大咖量化策略分享，全是干货，无闲聊 ，物以类聚,人以群分，一起感受思维碰撞的力量!
---------------------------------------------------------------

## 一、Ashare ( 开源 极简 A股实时行情数据API)
...
## 二、湘财证券的tushare智能金融平台

Tushare（Tushare‑Pro）是国内主流、面向量化爱好者的结构化金融数据 API 接口平台。
简单来讲就是金融数据库，你通过 Python 代码调用接口，一键拿到清洗完毕的股市各类数据，不用自己编写网页爬虫抓取行情。
返回的数据默认是 pandas 的 DataFrame 表格格式，可以直接拿来做选股、回测、因子分析，完美适配 PTrade 量化代码。

## Tushare 能干什么（全部可调取的数据大类）
- A 股基础资料：全部股票代码、上市日期、ST 标记、行业分类、停牌退市信息  
- 行情 K 线数据：日线、周月线、1/5/15/30/60 分钟线、复权价格、量价、涨跌停记录  
- 基本面财报：季报年报、营收、净利润、资产负债、市盈率、市净率  
- 高阶量化因子数据：筹码分布、龙虎榜、北向资金、机构调研、券商研报、游资明细、涨跌炸板数据  
- 场外品类：期货、期权、基金、可转债、港股、宏观经济、行业指数、新闻资讯  
- 适配你的 PTrade：在研究环境直接写代码调取数据，用来编写选股策略、策略回测、盘中条件判断。  

## 怎样免费使用Tushare
- 联系楼主，开通湘财的PTrade就可以获取token免费使用，开通PTrade只需账号超过5万资金即可，
- 开通 P‑Trade 之后券商下发，专属通道 token
- 无积分机制，绝大多数常用股票接口直接开放，不用做任务攒积分
- 原生适配 PTrade 研究环境、云策略；代码不需要额外安装 tushare 库，直接调用
- 湘财内网专线，调取行情速度更快、不容易触发官网限流、访问超时
- 主打 A 股量化刚需接口（行情、基础信息、龙虎榜、北向资金）




## 三、miniqmt+xtquant 券商级高速专业的行情数据交易软件及API接口
...
* [股民常用、人气高、对接券商多的7种量化软件工具及视频教程合集](https://github.com/CodeBang06/stockquant-suite-tutorial)
